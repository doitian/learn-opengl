...
int main(int argc, char *argv[])
{
  ...
  GLuint program = glCreateProgram();
  {
    ...
    ensureNoErrorMessage("Compiling Frag Shader", glrShaderSourceFromFile(fragShader, "shaders/c7-4.frag"));
  ...
  float mixValue = 0.2f;
  GLint mixValueLocation = glGetUniformLocation(program, "mixValue");
  ...
  const float mixValueSpeed = 0.5f;
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    {
      mixValue += deltaTime * mixValueSpeed;
      if (mixValue > 1.0f)
      {
        mixValue = 1.0f;
      }
